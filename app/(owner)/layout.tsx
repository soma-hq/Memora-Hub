...
/**
 * Owner layout providing sidebar, header and mobile navigation for admin pages
 * @param props - Component props
 * @param props.children - Nested page content
 * @returns The owner section layout with navigation shell
 */
export default function OwnerLayout({ children }: { children: React.ReactNode }) {
	...
	useEffect(() => {
		if (!currentUser && users.length > 0) {
			const elevatedUser = users.find((u) => u.roleId === "owner" || u.roleId === "marsha_teams") ?? users[0];
			setCurrentUser(elevatedUser.id);
		}
