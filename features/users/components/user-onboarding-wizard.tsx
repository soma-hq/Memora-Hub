...
const STEPS: Step[] = [
	...
	},
	{
		id: "squads",
		title: "Squads",
		description: "Rejoins les équipes de travail",
		icon: "users",
	},
	{
		id: "confirm",
...
export function UserOnboardingWizard({ onSuccess }: { onSuccess?: () => void }) {
	...
	const [userData, setUserData] = useState<UserData>({
		...
		password: "",
		role: "Collaborator",
		selectedSquads: ["grp-bazalthe"],
	});

	const updateField = useCallback((field: keyof UserData, value: unknown) => {
	...
	const validateStep = useCallback((): boolean => {
		...
		switch (currentStep) {
			...
			case 3: // Squads
				if (userData.selectedSquads.length === 0)
					errors.squads = "Sélectionne au moins une squad";
				break;
		}
	...
	const handleSubmit = async () => {
		...
		try {
			...
			const groupAccess = userData.selectedSquads.map((squadId) => {
				const squad = SQUADS.find((s) => s.id === squadId);
				return {
					groupId: squadId,
					groupName: squad?.label || squadId,
			...
			if (result.success) {
				toast.success(`Compte créé avec succès pour ${userData.selectedSquads.length} squad(s)`);
				onSuccess?.();
	...
	const toggleSquad = (squadId: string) => {
		setUserData((prev) => ({
			...prev,
			selectedSquads: prev.selectedSquads.includes(squadId)
				? prev.selectedSquads.filter((s) => s !== squadId)
				: [...prev.selectedSquads, squadId],
		}));
	};
								...
								<button
									...
									className={cn(
										...
										idx === currentStep
											? "bg-primary-500 w-6"
											: idx < currentStep
												? "bg-primary-500/50 w-1.5 cursor-pointer hover:bg-primary-500"
												: "w-1.5 bg-gray-700",
									)}
									title={step.title}
								/>
				...
				<div className="flex min-h-full flex-col items-center justify-center px-4 py-12 sm:px-6">
					<div className="w-full max-w-2xl space-y-8">
						...
						<div className="flex items-start gap-4">
							<div className={cn(
								"rounded-xl p-3",
								currentStep === 0 && "bg-gradient-to-br from-primary-500 to-blue-600",
								...
								<div className="space-y-3">
									{SQUADS.map((squad) => (
										<Card
											...
											className={cn(
												"cursor-pointer border transition-all",
												userData.selectedSquads.includes(squad.id)
													? "border-primary-500 bg-primary-500/10"
													: "border-gray-700/50 hover:border-gray-600",
											)}
										...
										>
											<div className="flex items-center gap-3">
												<Checkbox
													label={squad.label}
													checked={userData.selectedSquads.includes(squad.id)}
													onChange={() => {}}
												/>
											</div>
										</Card>
									...
									{formErrors.squads && (
										<p className="text-sm text-red-400">{formErrors.squads}</p>
									)}
								</div>
							)}
									...
									<Card padding="lg" className="border border-gray-700/50 bg-gray-800/30">
										<div className="space-y-3">
											...
											<div className="flex justify-between pt-2">
												<span className="text-sm text-gray-400">Squads</span>
												<div className="flex gap-1.5">
													{userData.selectedSquads.map((squadId) => {
														const squad = SQUADS.find((s) => s.id === squadId);
														return (
															<span
																key={squadId}
																className="inline-block rounded-full bg-primary-500/20 px-2.5 py-1 text-xs font-medium text-primary-300">
																{squad?.label}
															</span>
														);
													})}
