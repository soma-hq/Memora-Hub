...
const SECTIONS: Section[] = [
	...
	},
	{
		...
		content: (
			<div className="space-y-4">
				<Paragraph>
					L'application des sanctions suit une gradation stricte definie dans le Panel de Sanctions. Chaque
					sanction doit etre documentée et justifiée.
				</Paragraph>
				...
				<ul className="mt-2 space-y-1.5 text-sm text-gray-700 dark:text-gray-300">
					...
					<li className="flex items-start gap-2">
						<span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-gray-400" />
						La durée de la sanction si temporaire
					</li>
				</ul>
			</div>
		),
...
export function PolitiqueContent({ platform }: PolitiqueContentProps) {
	...
	useEffect(() => {
		const observer = new IntersectionObserver(
			...
			{ rootMargin: "-96px 0px -55% 0px", threshold: [0.1, 0.3, 0.5, 0.8] },
		);
	...
	return (
		<div className="flex flex-col gap-8 lg:flex-row">
			...
			<div className="min-w-0 flex-1 space-y-6">
				<Card padding="md" className="border-l-warning-500 border-l-4">
					<div className="flex items-start gap-2">
						<Icon name="warning" size="sm" className="text-warning-500 mt-0.5" />
						<p className="text-sm text-gray-700 dark:text-gray-300">
							Cette politique est la reference prioritaire. Les consignes operationnelles sont consideres
							comme annexes temporaires et ne peuvent pas contredire ce document.
						</p>
					</div>
				</Card>
