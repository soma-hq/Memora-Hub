...
export function useAbsenceActions(absences: Absence[], setAbsences: React.Dispatch<React.SetStateAction<Absence[]>>) {
	 ...
	 * @returns New absence entity, or throws if validation fails
	 */
	const createAbsence = useCallback(
		async (data: AbsenceFormData) => {
			const validation = container.managers.absence.validateRequest(data);
			if (!validation.valid) throw new Error(validation.error);

			setIsLoading(true);

			// TODO(Phase 31): replace with real server action call.
			// buildOptimisticAbsence uses the current user from session — userId/userName
			// should come from useDataStore().currentUser once mock data is removed.
			const newAbsence = container.managers.absence.buildOptimisticAbsence(
				data,
				"u-current",
				"Utilisateur courant",
			);
